...
function SignUpPage() {
    ...
    function isFormValid() {
        return username.length > 0 && validator.isEmail(username) && password.length > 7 && rePassword.length > 7
            && password === rePassword && firstName.length > 0 && lastName.length > 0;
    }

    const handleSubmit = async (ev) => {
        const result = await signup({ username: username, firstName: firstName, lastName: lastName, password: password });
        if (result)
            history.go(0);
    }
                    ...
                    >
                        <div className="flex flex-col items-center justify-center w-full py-48 px-12 max-w-216">
                            ...
                            <TextField
                                ...
                                InputProps={{
                                    endAdornment: (
                                        <InputAdornment position="end">
                                            {!showRePass ?
                                                <VisibilityOffIcon style={{ color: 'grey' }} className='cursor-pointer' onClick={() => setShowRePass(true)}></VisibilityOffIcon> :
                                                <VisibilityIcon style={{ color: 'grey' }} className='cursor-pointer' onClick={() => setShowRePass(false)}></VisibilityIcon>
                                            }
                                        </InputAdornment>
                                    )
                                }}
                            ...
                            {rePassword !== "" && rePassword !== password ?
                                <p className='text-red-700 mb-12 -mt-14'>Confirm Password must be same with Password</p>
                                : null
                            }
                            ...
                            <a href="/login" className='mt-8'>Already have an account?</a>
                        </div>
                    </Card>
